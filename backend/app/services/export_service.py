    ...
    for row in rows:
        writer.writerow(
            [
                ...
                row.get("project_display_score"),
                row.get("project_innovation_score"),
                row.get("key_highlight_score"),
                row.get("group_total_score"),
                row.get("member_scores_json"),
                row.get("total_score"),
                row.get("comment"),
                row.get("updated_at"),
            ]
...
def build_teacher_score_xlsx_bytes(summary_rows: Iterable[dict], score_rows: Iterable[dict]) -> bytes:
    ...
    ws_summary.append(
        [
            ...
            "score_count",
            "average_total_score",
            "average_group_total_score",
            "average_project_display_score",
            "average_project_innovation_score",
            "average_key_highlight_score",
            "average_member_personal_score",
            "average_five_scale_score",
            "updated_at",
        ]
    )
    for row in summary_rows:
        ws_summary.append(
            [
                ...
                row.get("average_group_total_score"),
                row.get("average_project_display_score"),
                row.get("average_project_innovation_score"),
                row.get("average_key_highlight_score"),
                row.get("average_member_personal_score"),
                row.get("average_five_scale_score"),
                row.get("updated_at"),
            ]
        )
